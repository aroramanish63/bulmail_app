...
function getBlogsList($catName, $archiveId) {
    ...
    $sql = "select t1.*, t2.catName from " . $GLOBALS['cfg']['db_prefix'] . "blog as t1 JOIN " . $GLOBALS['cfg']['db_prefix'] . "blog_category as t2 ON t1.categoryId = t2.id WHERE t1.status = 1 $extraQry ORDER BY t1.add_date DESC limit 3";
