...
if($_REQUEST['per_del_id'])
{
	...
	mysql_query("Update tab_version set int_del_status=10 where int_uid=" . $_SESSION["user_id"] . " And int_id=" . $_REQUEST['per_del_id']) or die(mysql_error());
...
if($_REQUEST['s'])
    {
     ...
     $select_data="SELECT * FROM `tab_version` as t1 WHERE t1.int_del_status<>10 And `txt_local_path` not in (select `txt_local_path` from users_data where int_uid='".$_SESSION['user_id']."' ) and  int_uid='".$_SESSION['user_id']."' and int_id=(select max(int_id) from tab_version where txt_local_path=t1.txt_local_path) and txt_file_name like '%".$_REQUEST['s']."%'   order by txt_file_name asc";
    ...
    if($_SESSION['sorting_opt']==2)
     $select_data="SELECT * FROM `tab_version` as t1 WHERE t1.int_del_status<>10 And `txt_local_path` not in (select `txt_local_path` from users_data where int_uid='".$_SESSION['user_id']."' ) and  int_uid='".$_SESSION['user_id']."' and int_id=(select max(int_id) from tab_version where txt_local_path=t1.txt_local_path)    order by txt_file_name asc";
    else if($_SESSION['sorting_opt']==3)
     $select_data="SELECT * FROM `tab_version` as t1 WHERE t1.int_del_status<>10 And `txt_local_path` not in (select `txt_local_path` from users_data where int_uid='".$_SESSION['user_id']."' ) and  int_uid='".$_SESSION['user_id']."' and int_id=(select max(int_id) from tab_version where txt_local_path=t1.txt_local_path)   order by txt_last_modified desc ";
    else
     $select_data="SELECT * FROM `tab_version` as t1 WHERE t1.int_del_status<>10 And `txt_local_path` not in (select `txt_local_path` from users_data where int_uid='".$_SESSION['user_id']."' ) and  int_uid='".$_SESSION['user_id']."' and int_id=(select max(int_id) from tab_version where txt_local_path=t1.txt_local_path)  order by int_file_size desc ";
     }
...
<div class="fleft width" style="margin-top:23px; margin-bottom:6px;">
<hr >
...
<div style="float:right; width:auto; ">
       <ul id="nav" class="dropdown dropdown-horizontal" title="Options">
                               ...
                               <ul class="position">
                                               ...
                                               <img  src="https://www.databagg.com/user/images/untrash.png" style="width: 10px; height: 10px; " align='texttop' /> 
                                               Untrash </a></li>
                                               <li><a  href="javascript:void(0);" onclick="verify_force(<?php echo $fetch_data['int_id']; ?>)"> 
                                               <img  src="https://www.databagg.com/user/images/delete.png" style="width: 10px; height: 10px; " align='texttop' /> 
...
<!--<img src="https://www.databagg.com/user/image/nothing_shared.jpg" /> -->
</div>
    <?php
    }
?>
...
{
    location.href="index_<?php echo $_SESSION['lang']; ?>.php?name_page=shared_data_<?php echo $_SESSION['lang']; ?>&dir="+id;
}
function autoHide()
