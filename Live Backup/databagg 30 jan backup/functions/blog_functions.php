...
function getBlogsList($catName, $archiveId, $pageno=0, $limit=0){
	...
	$sql = "select t1.*, t2.catName from cms_tbl_blog as t1 JOIN tbl_blog_category as t2 ON t1.categoryId = t2.id WHERE t1.status = 1 $extraQry ORDER BY t1.add_date DESC LIMIT $start, $limit";
...
function getBlogsListCount($catName, $archiveId){
	...
	$sql = "select t1.*, t2.catName from cms_tbl_blog as t1 JOIN tbl_blog_category as t2 ON t1.categoryId = t2.id WHERE t1.status = 1 $extraQry";
	$query = mysql_query($sql);
