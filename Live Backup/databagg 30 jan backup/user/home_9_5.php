...
if($_REQUEST['total_box_item'])
{
   ...
   // print_r($arr_del);
    ...
    {
        $fetch_pth="select txt_real_path,int_is_folder from users_data where int_fid='".$del."'";
        $result_fetch=mysql_query($fetch_pth);
        $fetch_path_real=mysql_fetch_array($result_fetch);
        
        if($fetch_path_real['int_is_folder']==1)
        {
            $delete_multiple_first="update users_data set int_del_status='1' where int_fid='".$del."'";
            mysql_query($delete_multiple_first)or die(mysql_error());
            $delete_multiple="update users_data set int_del_status=1 where txt_real_path like '".$fetch_path_real['txt_real_path']."/"."%'";
        }
        else
        $delete_multiple="update users_data set int_del_status='1' where int_fid='".$del."'";
...
if($_REQUEST['del_id'])
{
    if($_REQUEST['is_f']==1)
    {
        $fetch_pth="select txt_real_path,txt_file_name,int_pid from users_data where int_fid='".$_REQUEST['del_id']."'";
        $result_fetch=mysql_query($fetch_pth);
        $fetch_path_real=mysql_fetch_array($result_fetch);
        // $fetch_path_real['txt_real_path'];
        //make an entry on del table
       
        
        $delete_file_first="update users_data set int_del_status=1 where int_fid='".$_REQUEST['del_id']."'";
        mysql_query($delete_file_first) ;
        
        
        $delete_file="update users_data set int_del_status=1 where txt_real_path like '".$fetch_path_real['txt_real_path']."/"."%'";
    }
    else
    {
