...
if($_REQUEST['unshare_id'])
{
    ...
    if($_REQUEST['is_f']==1)
    {
        $fetch_pth="select txt_real_path from users_data where int_fid='".$_REQUEST['unshare_id']."'";
        $result_fetch=mysql_query($fetch_pth);
        $fetch_path_real=mysql_fetch_array($result_fetch);
        // $fetch_path_real['txt_real_path'];
         $delete_file_first="update users_data set int_del_status=0 where int_fid='".$_REQUEST['unshare_id']."'";
         mysql_query($delete_file_first)or die(mysql_error());
        $delete_file="update users_data set int_del_status=0 where txt_real_path like '".$fetch_path_real['txt_real_path']."/"."%'";
    }
    else
    {
    $delete_file="update users_data set int_del_status=0 where int_fid='".$_REQUEST['unshare_id']."'";
    }
    
    //$delete_multiple="update users_data set  int_del_status=0 where int_fid='".$_REQUEST['unshare_id']."'";
    mysql_query($delete_file)or die(mysql_error());
    $suc_msg="Untrash successfully.";
}
...
if($_REQUEST['s'])
    {
     ...
     $select_data="select * from users_data where int_uid='".$_SESSION['user_id']."' and txt_file_name like '%".$_REQUEST['s']."%' and int_status=1   and int_del_status=1 order by int_is_folder desc,txt_file_name asc";
    else if($_SESSION['sorting_opt']==3)
     $select_data="select * from users_data where int_uid='".$_SESSION['user_id']."' and txt_file_name like '%".$_REQUEST['s']."%' and int_status=1   and int_del_status=1 order by int_is_folder desc,txt_last_modified desc";
    else
     $select_data="select * from users_data where int_uid='".$_SESSION['user_id']."' and txt_file_name like '%".$_REQUEST['s']."%' and int_status=1  and int_del_status=1 order by int_is_folder desc,int_file_size desc";
    ...
    if($_SESSION['sorting_opt']==2)
     $select_data="select * from users_data where int_uid='".$_SESSION['user_id']."' and int_status=1   and int_del_status=1 order by int_is_folder desc,txt_file_name asc";
    else if($_SESSION['sorting_opt']==3)
     $select_data="select * from users_data where int_uid='".$_SESSION['user_id']."' and int_status=1   and int_del_status=1 order by int_is_folder desc,txt_last_modified desc";
    else
     $select_data="select * from users_data where int_uid='".$_SESSION['user_id']."' and int_status=1  and int_del_status=1 order by int_is_folder desc,int_file_size desc";
     }
