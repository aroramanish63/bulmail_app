...
if($_REQUEST['code'])
{
    ...
    {
    $select_exist_share="select * from tab_share_records where share_link='".$_REQUEST['code']."'";
    $required_pass=1;
    }
    else
...
if(isset($_REQUEST['pass']))
{
     ...
     $select_pass_info="select * from tab_share_records where share_link='".mysql_real_escape_string($_REQUEST['code'])."' and txt_password='".mysql_real_escape_string($_REQUEST['pass'])."' ";
    $result_pass_info=mysql_query($select_pass_info)or die(mysql_error());
    if(mysql_num_rows($result_pass_info)==1)
    {
    $_SESSION["is_pass"]=1;
    $required_pass=0;
    }
...
<?php
if(!isset($_SESSION['is_pass']) &&  $_REQUEST['type']=="web")
{
 ?>
<div class="white_box2">
<div class="white_box">
<form id="frm_pass" name="frm_pass" method="post" action="">
<label>Password</label>
<input type="password" class="passinput" name="pass" id="pass" />
<input type="hidden" name="type" value="web" />
<input type="hidden" name="code" value="<?php echo $_REQUEST['code']; ?>" />
<input type="button" value="" class="butviewfile" onclick="sub_pass_check()" />
</form>
...
<script>
function sub_pass_check()
{
    if(document.getElementById('pass').value=="")
    {
        alert("Please enter password to unlock this file");
        return false;
    }
    window.frm_pass.submit();
}
</script>
<?php
}
...
if(isset($_SESSION['is_pass']) || $required_pass==0)
{
?>
<div class="wrapper">
	<?php
