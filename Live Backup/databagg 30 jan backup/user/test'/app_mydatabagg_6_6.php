...
<?php
if($pid==0)
	$result=mysql_query("select * from users_data where int_uid='".$_SESSION['user_id']."' and int_pid=0 and int_del_status=0 And int_is_sync=" . $issync . " order by int_is_folder desc,txt_file_name asc") or die(mysql_error());
else
	$result=mysql_query("select * from users_data where int_uid='".$_SESSION['user_id']."' and int_status=1 and int_pid=" . $_REQUEST["pid"] . " order by int_is_folder desc,txt_file_name asc") or die(mysql_error());
...
?></h4>
	...
	echo setModifiedDate($row["int_mod_date"],$row["int_mod_time"]); ?>
     </h6>
</div>
<?php 
...
function dirStructure($pid)
	...
	for($i=count($arr)-1;$i>=0;$i--)
		echo "<a href='app_index.php?name_page=app_mydatabagg&pid=" . $arrid[$i] . "&issync=0'>" . $arr[$i] . ">> </a>";
	}
?>
<script>
function restrict()
