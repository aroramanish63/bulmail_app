...
else
{
    ...
    if($_SESSION['sorting_opt']==2)
     $select_data="SELECT *
FROM tab_share
WHERE tab_share.int_uid ='".$_SESSION['user_id']."'
AND int_is_shared=1 and txt_file_name like '%".$_REQUEST['s']."%'
ORDER BY int_is_folder DESC , txt_file_name ASC";
    else if($_SESSION['sorting_opt']==3)
     $select_data="SELECT *
FROM tab_share
WHERE tab_share.int_uid ='".$_SESSION['user_id']."'
...
<div class="icon_sep" >
    ...
    ?>
    <?php 	if($fetch_m_time["txt_file_type"]!="folder") echo "Size: ".setFileSize($fetch_m_time["int_file_size"]) . "<br>";  echo "Modified: ".setModifiedDate($fetch_m_time["int_mod_date"],$fetch_m_time["int_mod_time"]); ?></h6>
</div>
       ...
       <ul id="nav" class="dropdown dropdown-horizontal" title="Options">
                               ...
                               <ul class="position">
                                               ...
                                               <li><a  href="javascript:void(0);" onclick="verify(<?php echo $fetch_data['int_share_id']; ?>)"> 
