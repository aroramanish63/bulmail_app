...
if(isset($_REQUEST['time_update']))
{
    $f='../nas/uploads/' . $_SESSION['user_id'] . '/sync';
    total_space_used_for_sync($_SESSION['user_id'],$f);
}
...
if($_REQUEST['new_dir_id'])
{
    ...
    $err_msg="Please provide a name for the new folder.";
        ...
        {   
            ...
            $fpath=$_SESSION['sync_path']."/".$_REQUEST['new_dir_name'];
            $newarr=explode("/",$fpath);
            $fpath=substr($fpath,3,strlen($fpath));
		$uniquefolder=$newarr[5];
             $insert_sync_db="insert into tab_webactions set int_member='".$_SESSION["user_id"]."' ,txt_action='Added' ,txt_type='Dir' ,txt_folname='".trim($_SESSION["folname"])."' ,txt_fullpath='".$fpath."' , txt_uniquefolder='" . $uniquefolder . "'";
...
if($_REQUEST['new_ppt_id'])
{
    ...
    $err_msg="Please provide a name for the Power-Point file.";
        ...
        $fpath=$_SESSION['sync_path']."/".$_REQUEST['new_ppt_name'].".ppt";
         $newarr=explode("/",$fpath);
		$fpath=substr($fpath,3,strlen($fpath));
        $uniquefolder=$newarr[5];
...
if($_REQUEST['new_excel_id'])
{
    ...
    $err_msg="Please provide a name for the Excel file.";
        ...
        {   
          ...
          $fpath=$_SESSION['sync_path']."/".$_REQUEST['new_excel_name'].".xls";
              $newarr=explode("/",$fpath);
	$fpath=substr($fpath,3,strlen($fpath));
    	$uniquefolder=$newarr[5];
             $insert_sync_db="insert into tab_webactions set int_member='".$_SESSION["user_id"]."' ,txt_action='Added' ,txt_type='File' ,txt_folname='".trim($_SESSION["folname"])."' ,txt_fullpath='".$fpath."'  , txt_uniquefolder='" . $uniquefolder . "'";
...
if($_REQUEST['new_text_id'])
{
    ...
    $err_msg="Please provide a name for the Text file.";
        ...
        $fpath=$_SESSION['sync_path']."/".$_REQUEST['new_text_name'].".txt";
               $newarr=explode("/",$fpath);
	$fpath=substr($fpath,3,strlen($fpath));
    	$uniquefolder=$newarr[5];
             $insert_sync_db="insert into tab_webactions set int_member='".$_SESSION["user_id"]."' ,txt_action='Added' ,txt_type='File' ,txt_folname='".trim($_SESSION["folname"])."' ,txt_fullpath='".$fpath."'  , txt_uniquefolder='" . $uniquefolder . "'";
...
if($_REQUEST['new_word_id'])
{
    ...
    $err_msg="Please provide a name for the Word file.";
        ...
        {   
           ...
           $fpath=$_SESSION['sync_path']."/".$_REQUEST['new_word_name'].".doc";
              $newarr=explode("/",$fpath);
		$fpath=substr($fpath,3,strlen($fpath));
        $uniquefolder=$newarr[5];
             $insert_sync_db="insert into tab_webactions set int_member='".$_SESSION["user_id"]."' ,txt_action='Added' ,txt_type='File' ,txt_folname='".trim($_SESSION["folname"])."' ,txt_fullpath='".$fpath."'  , txt_uniquefolder='" . $uniquefolder . "'";
...
if($_REQUEST['del_id'])
{
        ...
        //die($_SESSION['sync_path']."/".$_REQUEST['del_id']);
        $pth=$_SESSION['sync_path']."/".$_REQUEST['del_id'];
        $newarr=explode("/",$pth);
        $uniquefolder=$newarr[5];
        $pth=substr($pth,3,strlen($pth));
          $insert_sync_db="insert into tab_webactions set int_member='".$_SESSION["user_id"]."' ,txt_action='Deleted' ,txt_type='Dir' ,txt_folname='".trim($_SESSION["folname"])."' ,txt_fullpath='".$pth."' , txt_uniquefolder='" . $uniquefolder . "'";
       //echo "<script>alert(\"" . $insert_sync_db . "\");</script>";
             mysql_query($insert_sync_db)or die(mysql_error());
        ...
        $uniquefolder=$newarr[5];
        $pth=substr($pth,3,strlen($pth));
        $insert_sync_db="insert into tab_webactions set int_member='".$_SESSION["user_id"]."' ,txt_action='Deleted' ,txt_type='File' ,txt_folname='".trim($_SESSION["folname"])."' ,txt_fullpath='".$pth."'  , txt_uniquefolder='" . $uniquefolder . "'";
             mysql_query($insert_sync_db)or die(mysql_error());
        }
...
$vl=directory_view($_SESSION['dir']);
        ...
        //if($_SESSION['total_level']==4)
       // {
        //    $fol_item=explode(">>",$vl);
        //    $_SESSION['folname']=$fol_item[2];
       // }
?>
...
{
?>
       <ul id="nav" class="dropdown dropdown-horizontal" style="margin: 0px;">
                     ...
                     ?>  
    <div class="update_btns fright" style="margin: 0px;">
...
?>
 ...
 <!--<a class="new" href="javascript:void(0);" onclick="togle();">New</a>  -->
    ...
    <div id="light" class="white_content" >
        <a style="margin-left: 575px;"  href = "javascript:void(0)" onclick ="reld();">
       ...
       <img src="image/close_u.png" />
        ...
        <h2 style="color:black ;padding-bottom: 5px;font-size: 20px;">Upload to 'Databagg'</h2>
            ...
            <form action="upload_sync.php" method="post" enctype="multipart/form-data">
        		...
        		<button id="px-submit" style="margin-top: 0px;" type="submit" onclick="document.getElementById('upd').value='1';">Upload</button>
        		<button id="px-clear" style="margin-top: 0px;" type="reset">Clear</button>
        	</form>
        ...
        function reld()
        {
            ...
            if(document.getElementById('upd').value=='1')
            window.location.href=window.location.href+"&time_update=1";
        }
        </script>
