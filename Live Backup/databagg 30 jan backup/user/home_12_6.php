...
include("../include_user/include_home/dir_functions.php");
include("../include_user/include_home/space_functions.php");
include("../include_user/include_home/rename_functions.php");
include("../include_user/include_home/delete_functions.php");
////session_start();
error_reporting(0);
//print_r($_SESSION);
...
$result_local_path=mysql_query($select_local_path_info) or die(mysql_error());
...
// print_r($_REQUEST);
  //$fetch_parent_data=get_parent_details($_REQUEST['par_id']);
      $fetch_current_data=get_parent_details($_REQUEST['del_id']);
     // echo $fetch_current_data['txt_local_path'];
      doDelete($_SESSION['user_id'],$fetch_current_data['txt_local_path'],'');
...
if($_REQUEST['file_name'])
{
    ...
    {
        if($_REQUEST['dir_p']=="" || $_REQUEST['dir_p']==0)
        {
         
      $oldname="home|".$_REQUEST['file_name_prev'];
      $newname="home|".$_REQUEST['file_name'];
      }
      else
      {
         $fetch_parent_data=get_parent_details($_REQUEST['dir_p']);
      ...
      //echo $oldname;
      //echo $newname;
       doRename($_SESSION['user_id'],$oldname,$newname);
    ...
    $err_msg="Name contains  illegal character.";
        ...
        {   
          ...
          //   $localpath=$fetch_parent_details['txt_local_path']."|".urldecode($_REQUEST['new_dir_name']);
            ...
            doFolderCreate($_SESSION['user_id'],$_REQUEST['parent_id'],$_REQUEST['new_dir_name'],'',date("Ymd"),date("His"),'','');
...
//             $localpath=$fetch_parent_details['txt_local_path']."|".urldecode($nm);
    ...
    $basic_path=$drive_name."/"."uploads/".$_SESSION['user_id']."/";
    ...
    copy("temp_files/temp.txt",$realpath1);
       ...
       doFileUpload($_SESSION['user_id'],$pid,urldecode($nm),'',date("Ymd"),date("His"),$realpath,'');
...
{  
    ...
    $select_data="select * from users_data where int_is_sync=0 and int_uid='".$_SESSION['user_id']."' and int_status=1 and txt_file_name like '%".$_REQUEST['s']."%' and int_del_status=0 and int_pid='".$_REQUEST['dir']."'  order by int_is_folder desc,txt_file_name asc";
    else if($_SESSION['sorting_opt']==3)
    $select_data="select * from users_data where int_is_sync=0 and int_uid='".$_SESSION['user_id']."' and int_status=1 and txt_file_name like '%".$_REQUEST['s']."%' and int_del_status=0 and int_pid='".$_REQUEST['dir']."'  order by int_is_folder desc,txt_last_modified desc";
    else
    $select_data="select * from users_data where int_is_sync=0 and int_uid='".$_SESSION['user_id']."' and int_status=1 and txt_file_name like '%".$_REQUEST['s']."%' and int_del_status=0 and int_pid='".$_REQUEST['dir']."'  order by int_is_folder desc,int_file_size desc ";
...
else
{
    ...
    if($_SESSION['sorting_opt']==2)
     $select_data="select * from users_data where int_is_sync=0 and int_uid='".$_SESSION['user_id']."' and int_status=1 and txt_file_name like '%".$_REQUEST['s']."%' and int_pid=0 and int_del_status=0 order by int_is_folder desc,txt_file_name asc";
    else if($_SESSION['sorting_opt']==3)
     $select_data="select * from users_data where int_is_sync=0 and int_uid='".$_SESSION['user_id']."' and int_status=1 and txt_file_name like '%".$_REQUEST['s']."%' and int_pid=0 and int_del_status=0 order by int_is_folder desc,txt_last_modified desc";
    else
     $select_data="select * from users_data where int_is_sync=0 and int_uid='".$_SESSION['user_id']."' and int_status=1 and txt_file_name like '%".$_REQUEST['s']."%' and int_pid=0 and int_del_status=0 order by int_is_folder desc,int_file_size desc";
     $result_data=mysql_query($select_data) or die(mysql_query());  
}
$imgsrc=0;
...
function dirStructure($pid)
	...
	for($i=count($arr)-1;$i>=0;$i--)
		echo "<a href='index.php?name_page=home&dir=" . $arrid[$i] . "'>" . $arr[$i] . "<img src='images/icon-arrow-blue.gif' style='margin-left:2px;'> </a>";
...
<div id="content_version" class="white_content_version">
<a style="margin-left: 340px;"  href = "javascript:void(0)" onclick = "document.getElementById('content_version').style.display='none'">
...
<hr />
<br />
 <img src="images/ajax-loader_dir_load.gif" /> <b style="color:#E85005"> Loading versions ... </b> 
...
?>
<div class="container fleft" style="min-height: 550px;">
<center>
 <?php
     if($err_msg)
     {
...
<!-- <link href="css/drag_drop_main.css" rel="stylesheet" type="text/css" />
        ...
        <script src="js/script_drag_drop.js"></script>  -->
         ...
         <div id="light" class="white_content" style="display:block;margin-top: 25px;margin-bottom: 25px;" >
       ...
       <img src="image/close_u.png" />
        ...
        <div id="main_container" style="margin-left:200px;">
        	<h2 style="color:#049cd4 ;padding-bottom: 5px;font-size: 16px;margin-left: 25px;">
            <img src="images/drop-files.png" />
            </h2>
        	<br />
 ...
 if(mysql_num_rows($result_data)>0)
        {
            ...
            {
                generate_code($fetch_data['int_fid'],0);
                ?>
...
<?php  if($fetch_data['txt_file_type']=="folder")
                  ...
                  { ?>
                 <a href="download_new.php?id=<?php echo $fetch_data['int_fid'];  ?>&path=<?php echo crypt($fetch_data['txt_file_name'],ENC_KEY); ?>"
...
<div class="icon_sep" >
    ...
    <?php //echo crypt($fetch_data['txt_file_name'],ENC_KEY); ?>
 ...
 <img  src="images/skin-icon-rename.png" style="width: 10px; height: 10px; " align='texttop' />&nbsp; rename </a>
                                               ...
                                               <?php
                                               if($fetch_data['txt_file_type']!="folder")
                                               {
                                               ?>
                                                <!-- <li><a  href="javascript:void(0);" onclick="document.getElementById('content_version').style.display='block'"> 
                                               <img  src="image/clock.gif" style="width: 10px; height: 10px; " align='texttop' />&nbsp;
                                               Prevoious version </a></li> -->
                                               <?Php
                                               }
                                               ?>
      ...
      <div class="share_btns">
       ...
       <?php if($fetch_data['txt_file_type']=="folder") { ?>  
                   ...
                   <?php } else { ?>  
                   <a title="Download This File" class="download" href="download_new.php?id=<?php echo $fetch_data['int_fid'];  ?>&path=<?php echo crypt($fetch_data['txt_file_name'],ENC_KEY); ?>"> Download </a>
                   <?php } ?>
       </a>
   <?php 
...
countcbox();
 ...
 <form id="del_form" name="del_form" method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
        ...
        <input type="hidden" value="" id="is_f" name="is_f" />
         <input type="hidden" value="<?php echo $_REQUEST['dir']; ?>" id="par_id" name="par_id" />
        </form>
    ...
    <input type="hidden" id="dir_p" name="dir_p" value="<?php echo $_REQUEST['dir']; ?>" />
...
<?php
                           function using_ie() 
                            ...
                            //document.getElementById('dtxt').innerHTML="Select file here";
                            //alert("sasads");
                            }
                            </script>

<?php 
include "tree_str.php";
?>
