...
if($_REQUEST['total_box_item'])
{
    ...
    $delete_multiple="update tab_share set  int_is_shared=0 where int_share_id in ($ids)";
...
{
    $delete_multiple="update tab_share set  int_is_shared=0 where int_share_id='".$_REQUEST['unshare_id']."'";
    mysql_query($delete_multiple)or die(mysql_error());
    $suc_msg="Unshared successfully.";
}
if($_REQUEST['priv_unshare_id'])
{
    $delete_multiple="delete from tab_share_records where int_fid='".$_REQUEST['priv_unshare_id']."'";
    mysql_query($delete_multiple);
    $suc_msg="Unshared successfully.";
}
...
{  
    ...
    $select_data="select * from users_data where int_uid='".$_SESSION['user_id']."' and int_status=1 and int_is_shared=1 and int_del_status=0 and int_pid='".$_REQUEST['dir']."'  order by int_is_folder desc,txt_last_modified desc";
     ...
     $select_data="select * from users_data where int_uid='".$_SESSION['user_id']."' and int_status=1 and int_is_shared=1 and int_del_status=0 and int_pid='".$_REQUEST['dir']."'  order by int_is_folder desc,int_file_size desc ";
...
else
{
    ...
    if($_SESSION['sorting_opt']==2)
      $select_data="SELECT * FROM tab_share,users_data WHERE tab_share.int_uid ='".$_SESSION['user_id']."'
AND tab_share.int_is_shared=1 and tab_share.txt_file_name like '%".$_REQUEST['s']."%'
and users_data.int_fid=tab_share.int_fid  ORDER BY tab_share.int_is_folder DESC , tab_share.txt_file_name ASC ";
    else if($_SESSION['sorting_opt']==3)
      $select_data="SELECT * FROM tab_share,users_data WHERE tab_share.int_uid ='".$_SESSION['user_id']."'
AND tab_share.int_is_shared=1 and tab_share.txt_file_name like '%".$_REQUEST['s']."%'
and users_data.int_fid=tab_share.int_fid  ORDER BY tab_share.int_is_folder DESC , tab_share.txt_file_name ASC ";
...
<script type="text/javascript" src="https://www.databagg.com/user/balloontip.js"></script>
<script>
	var xmlhttp;
function testXML(){
...
<?php
       // echo traverse_history_shared(); // maintain tree structure for traverse files and folders
        ?>
          ...
          <div class="fullcontainer" style="margin-bottom:42px;">
...
<div class="fleft"> <!--<input id="acceptTerms"  name="acceptTerms" type="checkbox" value="<?php echo $fetch_data['int_share_id']; ?>"  /> --> </div>

<div class="icon">


        <img src="<?php echo get_file_icon1($fetch_data['int_is_folder'],$fetch_data['txt_file_name']); ?>" />  </a>

</div>
<div class="icon_sep" >
	<h4 >
    <?php if(get_ext($fetch_data['txt_file_name'])=="pdf") { ?>
                   <!-- <a href="download.php?path=uploads/<?php echo $fetch_data['txt_file_name']; ?>"> -->
                    <?php 
                    }
                    else if($fetch_data['int_is_folder']==1)
                    {
                        ?>
                    <a href="javascript:void(0);" > 
                        <?php
                    }
                    else if(get_ext($fetch_data['txt_file_name'])=="jpeg" || get_ext($fetch_data['txt_file_name'])=="png" || get_ext($fetch_data['txt_file_name'])=="gif" || get_ext($fetch_data['txt_file_name'])=="jpg")
                    {
                    ?>
    ...
    <h6> <?php 
     $select_file_m_time="select int_file_size,txt_file_type,int_mod_date,int_mod_time from users_data where int_fid='".$fetch_data['int_fid']."'  ";
    $result_file_m_time=mysql_query($select_file_m_time);
    $fetch_m_time=mysql_fetch_array($result_file_m_time);
    
    ?>
    <?php 	if($fetch_m_time["txt_file_type"]!="folder") echo "Size: ".setFileSize($fetch_m_time["int_file_size"]) . "<br>";  echo "Modified: ".setModifiedDate($fetch_m_time["int_mod_date"],$fetch_m_time["int_mod_time"]); 
    ...
    ?></h6>
       ...
       <ul id="nav" class="dropdown dropdown-horizontal" title="Options">
                               ...
                               <li><a href="javascript:void(0);" class="delete_dropdownn"></a>
                               <ul class="position" style="width: 145px;">
                                                
                                               
                                               <li style="width: 100%;"><a  href="javascript:void(0);" onclick="verify(<?php echo $fetch_data['int_share_id']; ?>)"> 
                                               <img  src="images/icon_share.png" style="width: 10px; height: 10px; " align='texttop' /> 
                                               Unshare Public Link </a></li>
                                               <?php
                                               $fid=$fetch_data['int_fid'];
                                               $select_private_exist="select * from tab_share_records where int_fid='".$fid."'";
                                               $rsrc=mysql_query($select_private_exist);
                                               if(mysql_num_rows($rsrc)>0)
                                               {
                                               ?>
                                                <li style="width: 100%;"><a  href="javascript:void(0);" onclick="verify_private(<?php echo $fetch_data['int_fid']; ?>)"> 
                                               <img  src="images/icon_share.png" style="width: 10px; height: 10px; " align='texttop' /> 
                                               Unshare Private Link </a></li>
                                               <?php
                                               }
                                               ?>
...
?>
   ...
   <input type="hidden" id="unshare_id" name="unshare_id" />
    <input type="hidden" id="priv_unshare_id" name="priv_unshare_id" />
    <input type="hidden" value="<?php echo $_REQUEST['name_page']; ?>" id="name_page" name="name_page" />
   </form>
...
function verify_private(did)
{
    
    
    if(confirm("Are you sure to unshare this file. "))
    {
    document.getElementById('priv_unshare_id').value=did;
    document.unshare.submit();
    }
}
...
{
    location.href="index.php?name_page=shared_data_<?php echo $_SESSION['lang']; ?>&dir="+id;
}
function autoHide()
