...
$f='uploads/' . $_SESSION['user_id'] . '/sync';
$bytes = 0;
if(file_exists($f))
{
...
function create_dir($new_dir_name)
{
     $select_exist_folder="select * from users_data where txt_file_name='".$new_dir_name."' and int_uid='".$_SESSION['user_id']."' and int_del_status=0 and int_is_folder=1";
        ...
        {
                $path_folder="uploads/".$_SESSION['user_id']."/".$new_dir_name;
               ...
               mkdir("uploads/".$_SESSION['user_id']."/".$new_dir_name);
                $description='Directory Added: '."uploads/".$_SESSION['user_id']."/".$new_dir_name;
                    ...
                    {       $new_folder=$name."($i)";
                            ...
                            $insert_folder_new="insert into users_data (int_uid,int_pid,txt_file_name,int_file_size,txt_file_type,int_status,int_del_status,txt_last_modified,int_is_folder,int_is_shared,txt_shared_link,txt_sharer_emails,txt_real_path) 
                             values ('".$_SESSION['user_id']."',0,'".$new_folder."','0','folder',1,0,'".$last_time."',1,0,'','','".$path_folder."')";
                                ...
                                $description='Directory Added: '."uploads/".$_SESSION['user_id']."/".$new_folder;
                                log_entry($description);
                                break;
                    }
            }
...
{   
     ...
     $select_exist_folder="select * from users_data where txt_file_name='".$new_dir_full_name."' and int_uid='".$_SESSION['user_id']."' and int_is_folder=0 and int_del_status=0";
        ...
        if(mysql_num_rows($result_exist_folder)==0)
        {
                $path_folder="uploads/".$_SESSION['user_id']."/".$new_dir_full_name;
               $insert_folder="insert into users_data (int_uid,int_pid,txt_file_name,int_file_size,txt_file_type,int_status,int_del_status,txt_last_modified,int_is_folder,int_is_shared,txt_shared_link,txt_sharer_emails,txt_real_path) 
               values ('".$_SESSION['user_id']."',0,'".$new_dir_full_name."','0','".$ext_type."',1,0,'".$last_time."',0,0,'','','".$path_folder."')";
               mysql_query($insert_folder) or die(mysql_error());
               //mkdir("uploads/".$_SESSION['user_id']."/".$new_dir_name);
               if($ftype=="ppt")
               copy("temp_files/temp.ppt","uploads/".$_SESSION['user_id']."/".$new_dir_name.".ppt");
               if($ftype=="doc")
               copy("temp_files/temp.doc","uploads/".$_SESSION['user_id']."/".$new_dir_name.".doc");
               if($ftype=="xls")
               copy("temp_files/temp.xls","uploads/".$_SESSION['user_id']."/".$new_dir_name.".xls");
               if($ftype=="txt")
               copy("temp_files/temp.txt","uploads/".$_SESSION['user_id']."/".$new_dir_name.".txt");
                $description='Files Added: '."uploads/".$_SESSION['user_id']."/".$new_dir_name;
                log_entry($description);
        }
        else
        {
            ...
            for($i=1;$i<=$count;$i++)
            {
                    ...
                    {       $new_folder=$new_dir_full_name."($i)";
                            $path_folder="uploads/".$_SESSION['user_id']."/".$new_folder;
                            $insert_folder_new="insert into users_data (int_uid,int_pid,txt_file_name,int_file_size,txt_file_type,int_status,int_del_status,txt_last_modified,int_is_folder,int_is_shared,txt_shared_link,txt_sharer_emails,txt_real_path) 
                             values ('".$_SESSION['user_id']."',0,'".$new_folder."','0','".$ext_type."',1,0,'".$last_time."',0,0,'','','".$path_folder."')";
                                mysql_query($insert_folder_new) or die(mysql_error());
                                     if($ftype=="ppt")
                                    copy("temp_files/temp.ppt","uploads/".$_SESSION['user_id']."/".$new_folder);
                                    if($ftype=="doc")
                                   copy("temp_files/temp.doc","uploads/".$_SESSION['user_id']."/".$new_folder);
                                   if($ftype=="xls")
                                   copy("temp_files/temp.xls","uploads/".$_SESSION['user_id']."/".$new_folder);
                                   if($ftype=="txt")
                                   copy("temp_files/temp.txt","uploads/".$_SESSION['user_id']."/".$new_folder);
                                //mkdir("uploads/".$_SESSION['user_id']."/".$new_folder);
                                $description='Files Added: '."uploads/".$_SESSION['user_id']."/".$new_folder;
                                log_entry($description);
                                break;
                    }
            }
    ...
    $added_path=return_path();
     $select_exist_folder="select * from users_data where txt_file_name='".$new_dir_full_name."' and int_uid='".$_SESSION['user_id']."' and int_is_folder=0 and int_pid='".$pid."'";
        ...
        if(mysql_num_rows($result_exist_folder)==0)
        {
                ...
                $path_folder="uploads/".$_SESSION['user_id']."/".$added_path."/".$new_dir_full_name;
               $insert_folder="insert into users_data (int_uid,int_pid,txt_file_name,int_file_size,txt_file_type,int_status,int_del_status,txt_last_modified,int_is_folder,int_is_shared,txt_shared_link,txt_sharer_emails,txt_real_path) 
               values ('".$_SESSION['user_id']."','".$pid."','".$new_dir_full_name."','0','".$ext_type."',1,0,'".$last_time."',0,0,'','','".$path_folder."')";
               mysql_query($insert_folder) or die(mysql_error());
                                if($ftype=="ppt")
                                    copy("temp_files/temp.ppt","uploads/".$_SESSION['user_id']."/".$added_path."/".$new_dir_name.".ppt");
                                    if($ftype=="doc")
                                   copy("temp_files/temp.doc","uploads/".$_SESSION['user_id']."/".$added_path."/".$new_dir_name.".doc");
                                   if($ftype=="xls")
                                   copy("temp_files/temp.xls","uploads/".$_SESSION['user_id']."/".$added_path."/".$new_dir_name.".xls");
                                   if($ftype=="txt")
                                   copy("temp_files/temp.txt","uploads/".$_SESSION['user_id']."/".$added_path."/".$new_dir_name.".txt");
               //mkdir("uploads/".$_SESSION['user_id']."/".$added_path."/".$new_dir_name);
                $description='files Added: '."uploads/".$_SESSION['user_id']."/".$new_dir_full_name;
                log_entry($description);
        }
        else
        {
            ...
            for($i=1;$i<=$count;$i++)
            {
                    ...
                    {       $new_folder=$new_dir_full_name."($i)";
                            //$path_folder="uploads/".$new_folder;
                            $path_folder="uploads/".$_SESSION['user_id']."/".$added_path."/".$new_folder;
                            $insert_folder_new="insert into users_data (int_uid,int_pid,txt_file_name,int_file_size,txt_file_type,int_status,int_del_status,txt_last_modified,int_is_folder,int_is_shared,txt_shared_link,txt_sharer_emails,txt_real_path) 
                             values (1,'".$pid."','".$new_folder."','0','".$ext_type."',1,0,'".$last_time."',0,0,'','','".$path_folder."')";
                                mysql_query($insert_folder_new) or die(mysql_error());
                                //mkdir("uploads/".$new_folder);
                                if($ftype=="ppt")
                                    copy("temp_files/temp.ppt","uploads/".$_SESSION['user_id']."/".$added_path."/".$new_folder);
                                    if($ftype=="doc")
                                   copy("temp_files/temp.doc","uploads/".$_SESSION['user_id']."/".$added_path."/".$new_folder);
                                   if($ftype=="xls")
                                   copy("temp_files/temp.xls","uploads/".$_SESSION['user_id']."/".$added_path."/".$new_folder);
                                   if($ftype=="txt")
                                   copy("temp_files/temp.txt","uploads/".$_SESSION['user_id']."/".$added_path."/".$new_folder);
    ...
    $added_path=return_path();
     $select_exist_folder="select * from users_data where txt_file_name='".$new_dir_name."' and int_uid='".$_SESSION['user_id']."' and int_is_folder=1 and int_pid='".$pid."'";
        ...
        if(mysql_num_rows($result_exist_folder)==0)
        {
                ...
                $path_folder="uploads/".$_SESSION['user_id']."/".$added_path."/".$new_dir_name;
               $insert_folder="insert into users_data (int_uid,int_pid,txt_file_name,int_file_size,txt_file_type,int_status,int_del_status,txt_last_modified,int_is_folder,int_is_shared,txt_shared_link,txt_sharer_emails,txt_real_path) 
...
{   
     ...
     $select_exist_folder="select * from users_data where txt_file_name='".$new_dir_name."' and int_uid='".$_SESSION['user_id']."' and int_is_folder=1";
        ...
        if(mysql_num_rows($result_exist_folder)==0)
        {
               ...
               rename ("uploads/".$_SESSION['user_id']."/".$old_dir_name,"uploads/".$_SESSION['user_id']."/".$new_dir_name);
                if($isfol==0) 
               createThumbs("uploads/".$_SESSION['user_id']."/".$new_dir_name,"uploads/thumb/".$new_dir_name,60,60);
              // copy("temp_files/temp.txt","uploads/".$_SESSION['user_id']."/".$new_dir_name.".txt");
                $description='Files Renamed: '."uploads/".$_SESSION['user_id']."/".$new_dir_name;
                log_entry($description);
        }
        else
        {
...
{   
     ...
     $select_exist_folder="select * from users_data where txt_file_name='".$new_dir_name."' and int_uid='".$_SESSION['user_id']."' and int_is_folder=1 and int_pid='".$pid."'";
        ...
        if(mysql_num_rows($result_exist_folder)==0)
        {
               ...
               rename ("uploads/".$_SESSION['user_id']."/".$added_path."/".$old_dir_name,"uploads/".$_SESSION['user_id']."/".$added_path."/".$new_dir_name);
                if($isfol==0) 
               createThumbs("uploads/".$_SESSION['user_id']."/".$added_path."/".$new_dir_name,"uploads/thumb/".$new_dir_name,60,60);
              // copy("temp_files/temp.txt","uploads/".$_SESSION['user_id']."/".$new_dir_name.".txt");
                $description='Files Renamed: '."uploads/".$_SESSION['user_id']."/".$added_path."/".$new_dir_name;
                log_entry($description);
        }
        else
        {
