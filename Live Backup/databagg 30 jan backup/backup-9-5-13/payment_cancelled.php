...
if(isset($_GET['tx']) && !empty($_GET['tx'])){ 
	    ...
	    $paymentDetails = $payment_handler->getPaymentDetails($transactionID);
	    $email = $paymentDetails['txt_username'];
	    $status = $paymentDetails['payment_status'];  
	    $date = date("D-m-Y h:i:s", strtotime($paymentDetails['createdtime']));
}else{
...
<div style="clear:both;"></div>
 ...
 <div class="prnthspan">Print this page</div>
   ...
   <ul> 
     <li>Your Order has been cancelled<strong></strong></li>
