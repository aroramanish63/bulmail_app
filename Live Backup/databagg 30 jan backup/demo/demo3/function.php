...
function handle_mysql_error($query,$msg,$page)

{
    
    mail_error_query($query,$msg,$page);
    echo "<script>window.location.replace(\"index.php?name_page=$page\");</script>";
...
{
    $select_total_used="select sum(int_file_size) as total from users_data where int_uid='".$_SESSION['user_id']."' and int_del_status=0";
...
function allocatted_space()
{
    $select_space="select int_space_allotted from tab_members where int_id= '".$_SESSION['user_id']."'";
    $result_space=mysql_query($select_space);
    $fetch_space=mysql_fetch_array($result_space);
    return $fetch_space['int_space_allotted'];
}

function get_total_consumed_memmory($member_id)
{
...
function generate_code($id,$is_sync)
{
   ...
   $result_exist_share=mysql_query($select_exist_share) or die(mysql_error());
    ...
    $rid=random_id();
      $update_share_link="insert into  tab_share (txt_share_link,txt_file_name,txt_real_path,int_is_folder,int_uid,int_fid,int_is_sync)
      values('".$rid."','".mysql_real_escape_string($fetch_link['txt_file_name'])."','".mysql_real_escape_string($fetch_link['txt_real_path'])."','".$fetch_link['int_is_folder']."','".$_SESSION['user_id']."','".$id."','".$is_sync."')";
...
function generate_code_explorer($id,$is_sync,$mid)
{
   ...
   $result_exist_share=mysql_query($select_exist_share) or die(mysql_error());
    ...
    $rid=random_id();
      $update_share_link="insert into  tab_share (txt_share_link,txt_file_name,txt_real_path,int_is_folder,int_uid,int_fid,int_is_sync)
      values('".$rid."','".$fetch_link['txt_file_name']."','".$fetch_link['txt_real_path']."','".$fetch_link['int_is_folder']."','".$mid."','".$id."','".$is_sync."')";
...
{   
     ...
     $select_exist_folder="select * from users_data where txt_file_name='".$new_dir_full_name."' and int_uid='".$_SESSION['user_id']."' and int_is_folder=0 and int_del_status=0";
        ...
        else
        {
            ...
            for($i=1;$i<=$count;$i++)
            {
                        $select_exist_folder2="select * from users_data where int_is_folder=0 and int_del_status=0 and int_uid='".$_SESSION['user_id']."' and txt_file_name='"."($i)".$new_dir_full_name."'";
                        $result_exist_folder2=mysql_query($select_exist_folder2);
                        $counts=mysql_num_rows($result_exist_folder2);
                    if($counts==0)
                    {       $new_folder="($i)".$new_dir_full_name;
                            $path_folder="../nas/uploads/".$_SESSION['user_id']."/".$new_folder;
                            $insert_folder_new="insert into users_data (int_uid,int_pid,txt_file_name,int_file_size,txt_file_type,int_status,int_del_status,txt_last_modified,int_is_folder,int_is_shared,txt_shared_link,txt_sharer_emails,txt_real_path) 
...
function dirStructure_path($pid)
	{
	$cnt=0;
	while($pid!=0)
		{
...
function create_inner_file($new_dir_name,$pid,$ftype)
{
    ...
    $added_path=dirStructure_path($pid);
     $select_exist_folder="select * from users_data where txt_file_name='".$new_dir_full_name."' and int_uid='".$_SESSION['user_id']."' and int_is_folder=0 and int_pid='".$pid."'";
        ...
        else
        {
            ...
            for($i=1;$i<=$count;$i++)
            {
                        $select_exist_folder2="select * from users_data where int_is_folder=0 and int_pid='".$pid."' and int_uid='".$_SESSION['user_id']."' and txt_file_name='"."($i)".$new_dir_full_name."'";
                        $result_exist_folder2=mysql_query($select_exist_folder2);
                        $counts=mysql_num_rows($result_exist_folder2);
                    if($counts==0)
                    {        $new_folder="($i)".$new_dir_full_name;
...
{
    $pth_thumb='../nas/uploads/'.$_SESSION['user_id'].'/thumb';
    ...
    //return  $pth_thumb."/".$name;
     if($type1=="jpeg" || $type1=="png" || $type1=="gif" || $type1=="jpg" || $type1=="bmp")
    return  "images/image_icon.png";
