...
if(isset($_REQUEST["submitCreditCardInfo"])  &&  isset($_REQUEST["planIdText"]) && isset($_SESSION['user_id']))
{
	...
	if($_REQUEST['recurring'] == 1) // For Recurring
	{
	...
	$nvpStr =	"&PAYMENTACTION=$paymentType&AMT=$amount&CREDITCARDTYPE=$creditCardType&ACCT=$creditCardNumber".
				...
				"&COUNTRYCODE=$country&CURRENCYCODE=$currencyID&CUSTOM=$planId&EMAIL=$email".$nvpRecurring;
	 
	// Execute the API operation; see the PPHttpPost function above.
	$httpParsedResponseAr = PPHttpPost($methodToCall, $nvpStr);
	 
	if("SUCCESS" == strtoupper($httpParsedResponseAr["ACK"]) || "SUCCESSWITHWARNING" == strtoupper($httpParsedResponseAr["ACK"])) {
		//exit('Direct Payment Completed Successfully: '.print_r($httpParsedResponseAr, true));
		
		
		$recurrProfileId = urldecode($httpParsedResponseAr["PROFILEID"]);
		if($httpParsedResponseAr["AMT"]){
		...
		if(mysql_query($insert_query)){
			if(mysql_query("insert into users_transaction(`userID`,`transactionID`,`timeStamp`,`amount`,`recurrProfileId`) values ('$user_id','$transactionId','$timeStamp','$amount','$recurrProfileId')")){
...
<script type="text/javascript">
  $(document).ready(function(e) {
	...
	$("#recurringDiv").hide();
	$('#recurrCheck').change(function(){
        if(this.checked)
            $('#recurringDiv').fadeIn('slow');
        else
            $('#recurringDiv').fadeOut('slow');
    ...
    <div class="formmidpanel">
        ...
        <div class="fullformrow" id="creditCardDiv">
         ...
         <div class="formfullrow">
            ...
            <input type="checkbox" id="recurrCheck" name="recurring" value="1" style="margin-right:10px;"  />Enable
         </div>

         <div style="width:100%; float:left;" id="recurringDiv">
          <div class="formfullrow">
          <div style="width:100%; float:left; color:#333; font-weight:bold; color:
          #0096cf; font-size:18px; padding-bottom:10px;">Recurring Details</div>
            <div class="labeltext">
          ...
          <div class="formfullrow">
           ...
           <input type="text" name="billingFreq" value="1" class="smallinput2" />
          </div>
        </div>
