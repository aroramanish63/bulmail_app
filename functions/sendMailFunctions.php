...
class sendMailFunctions extends AllFunctions{
     ...
     public function getAllemails(){
         ...
         if(is_array($result)){
             $str = '';
                  $i = 1;
            $str .= '<p><label>Select Emails</label><div class="multiselect input-short"><label><input type="checkbox" value="all" name="allemail" id="allemail" onclick="checkEmailAll(this,\'emails[]\');" />&nbsp;All Emails</label>';
                foreach($result as $all){
                   $str .= '<label><input type="checkbox" onclick="handleCheck(this);" value="'.$all['id'].'" name="emails[]" id="emails'.$i.'" />&nbsp;'.$all['email_id'].'</label>';
                   $i++;
                }
                $str .= '</div></p>';
               echo $str;                     
